...
The `cfips` column and `state_code` have a completely positive correlation. So, we will remove the redundant feature `state_code`. Also, `pct_bb` strongly correlates with `median_hh_inc` and `pct_college`. Also, `pct_college` strongly correlates with `microbusiness_density`, `median_hh_inc` and `pct_bb`. As features with moderate strong and strong correlation are redundant, we will remove these features (`pct_bb` and `pct_college`) before further analysis. Now, we draw the new correlation plot.

```{r}

# Select the columns with numeric data (not including "state_code", "pct_bb", "pct_college")
numeric_cols <- c("cfips",  "region_code", "microbusiness_density", "active", "year", "month", "pct_foreign_born", "pct_it_workers", "median_hh_inc")
...
model <- randomForest(microbusiness_density ~ cfips + region_code + active + year + month + pct_foreign_born + pct_it_workers + median_hh_inc, 
                      data = train_data)
...
# Converting ‘microbusiness_density’ column to a factor datatype
set.seed(92)

# Training ‘random forest’ model 
model <- train(microbusiness_data ~., data = merged_df_new,method = 'rf',trControl = trainControl(method = 'cv',number = 10))
model
#Building the model using another library to visualize
x <- ctree(microbusiness_data ~ . , data= merged_df_new)
plot(x, type="simple")
```

```{r}
# subset the data to include only the columns of interest
df <- merged_df_new[c("cfips", "region_code", "microbusiness_density", "active", "year", "month", "pct_foreign_born", "pct_it_workers", "median_hh_inc")]

# separate predictor and response variables
X <- as.matrix(df[, -4]) # exclude "active" column
y <- as.matrix(df[, 4])  # only include "active" column
...
# perform coordinate descent to solve Lasso optimization problem
while(TRUE) {
  beta_old <- beta
  
  for(j in 1:ncol(X)) {
    aj <- 2 * sum(X[, j]^2)
    bj <- 2 * sum(X[, j] * (r + X[, j] * beta[j]))
    if(bj < -lambda) {
      beta[j] <- (bj + lambda) / aj
    } else if(bj > lambda) {
      beta[j] <- (bj - lambda) / aj
    ...
    r <- r + X[, j] * (beta_old[j] - beta[j])
  }
  
  if(max(abs(beta - beta_old)) < eps) {
    break
  }
}

# print the coefficients and their corresponding variable names
lasso_coef <- cbind(beta, colnames(X))
colnames(lasso_coef) <- c("Coefficients", "Variable")
lasso_coef <- data.frame(lasso_coef)

# convert the coefficients column to numeric
lasso_coef$Coefficients <- as.numeric(lasso_coef$Coefficients)
#lasso_coef <- lasso_coef[order(-lasso_coef$Coefficients),]

# plot the coefficients
ggplot(lasso_coef, aes(x = reorder(Variable, Coefficients), y = Coefficients)) +
  geom_bar(stat = "identity") +
...
# plot the coefficients
#barplot(lasso_coef$Coefficients, names.arg = lasso_coef$Variable, las = 2, col="steelblue", main="Lasso Regression Coefficients") 
lasso_coef
```
...
ts_data <- ts(merged_df_new$microbusiness_density, start = c(2019, 8), frequency = 12)
...
legend("topright", legend = c("Forecast", "Actual"), col = c("red", "blue"), lty = 1)
...
train_data <- window(ts_data, end = c(2022, 2))
test_data <- window(ts_data, start = c(2022, 3))

# Train neural network autoregression model
nnetar_model <- nnetar(ts_data)

# Make forecasts using the trained model
forecast_data <- forecast(nnetar_model, h = length(test_data))
...
# Estimate parameters
fit <- merged_df_new |>
  forecast::model(ETS(microbusiness_density ~ error("A") + trend("N") + season("N")))
fc <- fit |>
  forecast(h = 5)
...
plot(forecast_data, main = "Neural Network Autoregression Forecasts")
...
```{r}
merged_df_new |>
  GGally::ggpairs(columns = c(9,10))
```

```{r}
merged_df_new |>
  GGally::ggpairs(columns = c(11:15))
```

```{r}
merged_df_new |>
  GGally::ggpairs(columns = c(6,7))
```
...
# Scatter Plot
ggplot(merged_df_new, aes(x=pct_bb, y=median_hh_inc)) +
  geom_point() +
